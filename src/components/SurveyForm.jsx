...
import { getSheetConfig } from '../utils/storage';

const SurveyForm = ({ isConfigured }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      try {
        await submitToGoogleSheets(config.sheetUrl, config.sheetName, submissionData);
        clearSurveyDraft();
        setIsCompleted(true);
      } catch (error) {
        ...
        {!isConfigured && (
          <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-sm text-yellow-800">
              <strong>Warning:</strong> Google Sheets is not configured. Responses will be saved locally only.
            </p>
          </div>
        )}
